...
export default function DocumentOCR() {
  ...
  //       const [embeddedPdf] = await pdfDoc.embedPdf(pdfDoc);
  ...
  //       const PDFWidth = embeddedPdfDims.width;
  //       const PDFHeight = embeddedPdfDims.height;
  //       const PDFXMargin = 0;
  //       const PDFYMargin = 10;
  //       const PDFMargin = PDFXMargin + PDFYMargin;

  //       const PDFPosX = pageWidth;
  //       const PDFPosY = PDFXMargin;

  //       await Promise.all(
  //         pages.map(async (page) => {
  //           // const embedPage = await pdfDoc.embedPage(page);
  //           const newPage = pdfDoc.addPage();
  //           newPage.drawPage(embeddedPdf, {
  //             x: PDFPosX,
  //             y: PDFPosY,
  //             rotate: degrees(-270),
  //           });
  ...
  const handleFileRecognition = async () => {
    ...
    try {
      ...
      QRCode.toDataURL(OCRData, { width: 300 }, async (err, dataUrl) => {
        ...
        // Load the PDFDocument from the ArrayBuffer
        const pdfInput = await PDFDocument.load(pdfBuffer);
        const pdfDoc = await PDFDocument.create();
        const [embeddedPdf] = await pdfDoc.embedPdf(pdfInput);
        const embeddedPdfDims = embeddedPdf.scale(1);
        ...
        // // Get dimensions of the first page or document
        // const pageWidth = firstPage.getWidth();
        // const pageHeight = firstPage.getHeight();

        // // Positioning constants for QR code and text
        // const PDFXMargin = 0;
        // const PDFYMargin = 10;
        // const PDFPosX = pageWidth;
        // const PDFPosY = PDFXMargin;

        // Embed each page from original PDF into new PDF
        await Promise.all(
        ...
        // Embed QR code image into new PDF
        const pngUrl = dataUrl;
        const pngImageBytes = await fetch(pngUrl).then((res) =>
          res.arrayBuffer()
        );
        ...
        // Embed text using HelveticaBold font
        const boldHelveticaFont = await pdfDoc.embedFont(
          StandardFonts.HelveticaBold
        );
        const txtWidth = boldHelveticaFont.widthOfTextAtSize(textValue, 6);
        ...
        const txtPosY = pageHeight - txtYMargin;
        firstPage.drawText(textValue, {
          x: txtPosX,
          y: txtPosY,
          size: 6,
        ...
        // Positioning constants for QR code image
        const imageXMargin = 0;
        const imageYMargin = 10;
        const imagePosX = pageWidth - pngDims.width - imageXMargin;
        const imagePosY = imageYMargin;

        // Draw QR code image on first page of document
        firstPage.drawImage(pngImage, {
          x: imagePosX,
          y: imagePosY,
          width: pngDims.width,
